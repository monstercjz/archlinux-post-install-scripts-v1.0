...
# 说明: 这是所有 log_info/log_error 等函数的底层实现。
...
_log_message_core() {
    ...
    # caller 0 返回 'LINE FILE FUNCTION'。我们取第二个字段 (FILE)。
    # 这样可以更准确地获取原始调用脚本的文件名，即使是通过多层 sourced 调用的。
    local calling_script_info=$(caller 0)
    local calling_script_file=$(echo "$calling_script_info" | awk '{print $2}')
    local calling_script_name=$(basename "${calling_script_file:-$0}")

    # 根据日志级别选择颜色
    local terminal_color_code="${COLOR_RESET}" # 默认无色
    case "$level" in
        ...
        *)         terminal_color_code="${COLOR_RESET}" ;; # 未知级别保持无色
    esac

    # 1. 终端输出：带颜色的日志信息。
    # 根据 main_config.sh 中的 ENABLE_COLORS 变量决定是否输出颜色。
    # ${ENABLE_COLORS:-true} 提供默认值，防止 main_config.sh 未加载或变量不存在。
    if [[ "${ENABLE_COLORS:-true}" == "true" ]]; then
        echo -e "${terminal_color_code}[$timestamp] [$level] [$calling_script_name] $message${COLOR_RESET}"
    else
        echo "[$timestamp] [$level] [$calling_script_name] $message"
    fi
    
    # 2. 文件写入：纯文本日志信息。
    # 确保 CURRENT_SCRIPT_LOG_FILE 已被设置。
    # 使用 ${VAR:-} 形式，确保变量即使未设置，也不会导致 'unbound variable' 错误。
    if [[ -n "${CURRENT_SCRIPT_LOG_FILE:-}" ]]; then # 确保日志文件路径非空
        # 将日志内容通过管道传递给 tee，tee 将以当前脚本的权限 (root) 写入文件。
        # 文件所有权在 setup_logging 中调整。
        # `> /dev/null` 确保 tee 的标准输出被丢弃，避免重复打印到终端。
        echo "[$timestamp] [$level] [$calling_script_name] $message" | \
...
_initialize_project_environment() {
    ...
    if ! ensure_log_dir_user_owned "$LOG_ROOT" "$ORIGINAL_USER"; then
...
log_debug() {
    # ${DEBUG_MODE:-false} 确保 DEBUG_MODE 未设置时默认为 false
...
# ==============================================================================
# 日志目录权限管理函数 (由 _initialize_project_environment 调用)
# ------------------------------------------------------------------------------

# ensure_log_dir_user_owned()
...
ensure_log_dir_user_owned() {
    ...
    # 检查 ORIGINAL_USER 是否对目录有写权限
    # `sudo -u "$user" test -w "$dir_path"` 检查指定用户是否对目录有写权限
    if ! sudo -u "$user" test -w "$dir_path"; then
        ...
        if command -v setfacl &>/dev/null; then
            ...
            # 这里不使用 sudo -u，因为 setfacl 是 root 权限操作
