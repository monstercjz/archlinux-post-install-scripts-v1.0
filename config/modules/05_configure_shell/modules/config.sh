...
configure_theme() {
    ...
    if grep -qE '^\s*ZSH_THEME=' "$ZSHRC_FILE"; then
        ...
        if [ "$EUID" -eq 0 ] && [ -n "$ORIGINAL_USER" ]; then
             # 确保文件所有权正确
             run_command sudo chown "$ORIGINAL_USER:$ORIGINAL_USER" "$ZSHRC_FILE" || log_warn "无法更改 $ZSHRC_FILE 的所有权"
        ...
        if $sed_success; then
             log_info "成功将 ZSH_THEME 更新为 Powerlevel10k。"
             return 0
        else
             log_error "使用 sed 更新 ZSH_THEME 失败！请手动修改 $ZSHRC_FILE。"
             # 恢复备份 (如果存在)
        ...
        log_info "未找到 ZSH_THEME 设置，在 $ZSHRC_FILE 末尾添加 Powerlevel10k 主题设置。"
        ...
        if $append_success; then
             log_info "成功添加 Powerlevel10k 主题设置。"
             return 0
        else
             log_error "向 $ZSHRC_FILE 添加 ZSH_THEME 失败！"
             return 1
        fi
    fi
}
...
configure_plugins() {
    ...
    log_info "期望启用的插件: ${UNIQUE_DESIRED_PLUGINS[*]}"
    ...
    if [[ -n "$plugins_line" ]]; then
        ...
        log_info "找到现有的插件列表: ${current_plugins[*]}"
    else
        log_warn "未在 $ZSHRC_FILE 中找到有效的 'plugins=(...)' 行。"
    ...
    if $modified || ! $plugins_line_found_in_original; then
        if [ ${#added_plugins[@]} -gt 0 ]; then
             log_info "添加以下新插件到列表: ${added_plugins[*]}"
        elif ! $plugins_line_found_in_original; then
             log_info "将在文件末尾添加新的插件列表。"
        else
             log_info "插件列表无需添加新插件，但可能需要重新格式化或写入。"
        fi
        ...
        if ! $processed; then
             log_info "在文件末尾添加新的插件列表: ${current_plugins[*]}"
        ...
        if [ "$EUID" -eq 0 ] && [ -n "$ORIGINAL_USER" ]; then
             # 先设置所有权再移动
             run_command sudo chown "$ORIGINAL_USER:$ORIGINAL_USER" "$temp_zshrc" || log_warn "无法更改临时文件的所有权"
        ...
        if $mv_success; then
            log_info "插件配置成功更新到 $ZSHRC_FILE。"
        else
            log_error "无法将插件更改写回 $ZSHRC_FILE！"
            rm -f "$temp_zshrc" # 尝试清理临时文件
            return 1
        fi
    else
        log_info "插件列表已包含所有期望的插件，且无需修改 $ZSHRC_FILE。"
...
configure_aliases_and_extras() {
    log_notice "配置别名和其他设置..."

    if [ ! -f "$ZSHRC_FILE" ]; then
        log_warn "$ZSHRC_FILE 文件不存在。无法配置别名。"
        return 1
    fi
    ...
    if command_exists fzf; then
         ...
         if ! grep -qF 'export FZF_DEFAULT_OPTS=' "$temp_zshrc"; then
             log_info "添加 FZF 默认选项配置。"
             echo -e "\n# FZF 配置" >> "$temp_zshrc"
             echo "$fzf_options_export" >> "$temp_zshrc"
             changes_made=true
         else
             log_info "FZF_DEFAULT_OPTS 配置已存在。"
         fi
    ...
    if is_omz_plugin_installed "fzf-tab"; then
        log_info "检测到 fzf-tab 插件已安装，检查其 zstyle 配置..."
...
# End fzf-tab configuration"
        ...
        if ! grep -qF "zstyle ':fzf-tab:*' fzf-flags" "$temp_zshrc"; then
            log_info "添加 fzf-tab 的 zstyle 配置..."
            echo -e "\n${fzf_tab_config_block}" >> "$temp_zshrc"
            changes_made=true
        else
            log_info "fzf-tab 的 zstyle 配置似乎已存在。"
        fi
    fi
    ...
    if $changes_made; then
        if mv "$temp_zshrc" "$ZSHRC_FILE"; then
            log_info "别名和其他配置成功更新到 $ZSHRC_FILE。"
            chmod 644 "$ZSHRC_FILE"
            return 0
        else
            log_error "无法将别名更改写回 $ZSHRC_FILE！"
            rm -f "$temp_zshrc"
            return 1
        fi
    else
        log_info "无需添加新的别名或额外配置。"
        rm -f "$temp_zshrc" # 清理临时文件
        return 0
    fi
}
...
# 主配置函数
run_configuration() {
    log_notice "开始配置 Zsh 环境 (目标: $ZSHRC_FILE)..."

    # 备份 .zshrc
    # backup_file 内部需要处理 sudo
    if ! backup_file "$ZSHRC_FILE"; then
        log_error "备份 $ZSHRC_FILE 失败！配置中止。"
        return 1
    fi
    ...
    if [ ! -f "$ZSHRC_FILE" ]; then
        log_info "$ZSHRC_FILE 不存在，将创建它。"
        touch "$ZSHRC_FILE" || { log_error "创建 $ZSHRC_FILE 失败！"; return 1; }
    fi
    if [ "$EUID" -eq 0 ] && [ -n "$ORIGINAL_USER" ]; then
        run_command sudo chown "$ORIGINAL_USER:$ORIGINAL_USER" "$ZSHRC_FILE" || log_warn "无法设置 $ZSHRC_FILE 的所有权"
        if [ -f "$P10K_CONFIG_FILE" ]; then
             run_command sudo chown "$ORIGINAL_USER:$ORIGINAL_USER" "$P10K_CONFIG_FILE" || log_warn "无法设置 $P10K_CONFIG_FILE 的所有权"
        fi
    fi
    ...
    if $config_ok; then
        log_info "Zsh 配置完成。请重新启动 Zsh 或运行 'source ~/.zshrc' 来应用更改。"
        # 提示用户运行 p10k configure
        if command_exists p10k; then
             log_info "Powerlevel10k 已安装。为了获得最佳体验，建议运行 'p10k configure' 进行个性化配置。"
        fi
        return 0
    else
        log_error "Zsh 配置过程中遇到错误。请检查上面的日志并手动检查 $ZSHRC_FILE 文件。"
        return 1
    fi
}
