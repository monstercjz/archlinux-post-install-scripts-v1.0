...
declare -A COMMON_SOFTWARE_INSTALLATION_MENU_ENTRIES=(
    ...
    [1]="基础命令行软件 (fd tar ncdu reflector 等)|action:core_modules:00_common_software_installation/01_install_essential_software.sh"
...
main() {
    log_info "Starting Common Software Installation Menu loop."
    ...
    log_info "Starting Common Software Installation Menu loop ended."
}
...
exit_script() {
    local exit_code=${1:-0}
    log_info "Exiting Starting Common Software Installation Menu script."
    exit "$exit_code"
}
