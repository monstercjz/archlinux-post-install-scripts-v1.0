        ...
        if [[ -f "$file" ]]; then
            ...
            if [[ $? -eq 0 ]]; then
                ...
                if [[ "$filename" == *.hook ]]; then
                    ...
                    # 使用 sed 替换 /path/to/your/archlinux-post-install-scripts-v1.0 为实际的 PROJECT_ROOT
                    sed -i "s|/path/to/your/archlinux-post-install-scripts-v1.0|$PROJECT_ROOT|g" "$target_path"
                    if [[ $? -eq 0 ]]; then
