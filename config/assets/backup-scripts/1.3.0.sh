...
cat <<EOF
# Default configuration for ${SCRIPT_NAME}
# Generated on $(date) by script version ${SCRIPT_VERSION}
...
# 日志文件设置
# CONF_LOG_FILE:
#   如果 CONF_LOG_TIMESTAMPED="false" (默认), 此为固定日志文件的完整路径。
#   如果 CONF_LOG_TIMESTAMPED="true", 此为存放带时间戳日志文件的目录路径。
CONF_LOG_FILE="/var/log/$(basename "$SCRIPT_NAME" .sh)" # 示例: /var/log/arch_backup (目录) 或 /var/log/arch_backup.log (文件)
...
CONF_LOG_LEVEL="INFO"                       # DEBUG, INFO, WARN, ERROR, FATAL_ERROR

# === 用户特定备份设置 ===
CONF_TARGET_USERNAME=""                     # 指定备份家目录的用户, 如 "myuser". 留空则自动判断.
...
CONF_BACKUP_PACKAGES="true"
CONF_BACKUP_LOGS="true"
CONF_BACKUP_CUSTOM_PATHS="false"

# === 用户数据配置 (CONF_BACKUP_USER_DATA="true") ===
...
# === 自定义路径配置 (CONF_BACKUP_CUSTOM_PATHS="true") ===
CONF_CUSTOM_PATHS_INCLUDE=(
    # "/opt/my_app/data"
    # "/srv/web_content"
)
...
# === 系统日志配置 (CONF_BACKUP_LOGS="true") ===
CONF_SYSTEM_LOG_FILES=( "pacman.log" "Xorg.0.log" )
CONF_BACKUP_JOURNALCTL="true"
CONF_JOURNALCTL_ARGS="" # 例如: "--since yesterday" 或 "--boot=-1"

# === 备份机制 ===
CONF_INCREMENTAL_BACKUP="true"
CONF_COMPRESSION_ENABLE="true"
CONF_COMPRESSION_METHOD="xz" # gzip, bzip2, xz
CONF_COMPRESSION_LEVEL="6"   # 1-9 for xz/gzip
# CONF_COMPRESSION_EXT 会自动根据 METHOD 设置，无需手动配置

# === 保留策略 ===
CONF_RETENTION_UNCOMPRESSED_COUNT="3"    # 保留最近N个未压缩快照 (至少1个用于增量)
CONF_RETENTION_COMPRESSED_COUNT="10"   # 保留最近N个压缩归档 (0禁用)
CONF_RETENTION_COMPRESSED_DAYS="90"    # 删除超过N天的压缩归档 (0禁用)

# === 高级功能 ===
CONF_PARALLEL_JOBS="1" # >1 需要 GNU Parallel
CONF_MIN_FREE_DISK_SPACE_PERCENT="10"

# === 资源使用控制 ===
CONF_USE_NICE="false"       # "true" to use nice for rsync/tar
CONF_NICE_LEVEL="10"        # nice level (0 high prio, 19 low prio)
CONF_USE_IONICE="false"     # "true" to use ionice for rsync/tar
CONF_IONICE_CLASS="2"       # ionice class: 1 (realtime), 2 (best-effort), 3 (idle)
CONF_IONICE_LEVEL="4"       # ionice level for best-effort (0-7, lower is more prio)

# === 用户交互 ===
CONF_PROMPT_FOR_CONFIRMATION="true" # "false" for non-interactive/cron usage
EOF
}
